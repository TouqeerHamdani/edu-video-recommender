...
async def _execute_text_search(session, query, min_duration, max_duration, limit):
    ...
    """
    # Sentinel Security Fix: Replaced raw string interpolation (duration_filter_sql)
    # with parameterized inputs to prevent potential SQL injection.
    duration_filter = ""
    if min_duration is not None:
        duration_filter += " AND duration >= :min_duration"
    if max_duration is not None:
        duration_filter += " AND duration < :max_duration"
    ...
    try:
        ...
        if query_vector is not None:
            ...
            supply_sql = f"""
                ...
                LIMIT :limit
            """
            params = {"qe": str(embedding_list), "limit": top_n}
            if min_duration is not None:
                params["min_duration"] = min_duration
            if max_duration is not None:
                params["max_duration"] = max_duration
