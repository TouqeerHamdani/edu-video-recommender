...
async def _execute_text_search(session, query, min_duration, max_duration, limit):
    ...
    AND (CAST(:min_duration AS INTEGER) IS NULL OR duration >= CAST(:min_duration AS INTEGER))
    AND (CAST(:max_duration AS INTEGER) IS NULL OR duration < CAST(:max_duration AS INTEGER))
    ORDER BY view_count DESC NULLS LAST, like_count DESC NULLS LAST
    LIMIT :limit
    """
    return await session.execute(
        text(sql),
        {"query": query, "min_duration": min_duration, "max_duration": max_duration, "limit": limit}
    )
